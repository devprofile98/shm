...
std::shared_ptr<shader> Actor::setShader(const char *v_shader,
                                         const char *f_shader) {
  this->m_object_shader =
      SHM::Engine::GetEngine()->CreateShader(v_shader, f_shader);
...
Actor::setShader(std::shared_ptr<shader> source_shader) {
...
void Actor::loadModel(const char *file_path) {
  this->model = SHM::Engine::GetEngine()->getRenderer()->LoadModel(
      file_path, this->m_object_shader);
  //   if (this->model) {
