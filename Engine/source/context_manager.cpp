...
ContextManager::ContextManager(const char* window_name){
    createWindow(960, 540, window_name);
}
...
void ContextManager::FrameBufferCallback(GLFWwindow* window, int width, int height){
...
void ContextManager::createWindow(uint32_t width, uint32_t height, const char* window_name){
    ...
    std::cout << "OPENGL context created with information:"
              <<major<<" "
              <<minor
...
GLFWwindow* ContextManager::GetWindow() const{
    return this->window;
}
