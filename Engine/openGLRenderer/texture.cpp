...
unsigned int Texture::createTexture2D(GLint format, uint16_t width, uint16_t height, const GLvoid *data) {
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
    // std::cout << "Engine::TEXTURE::DEBUG::-> Depth texture succesfully in slot number " << Model::texture_layout_counter
    //           << " with id: " << texture_id << std::endl;
    SHM::Logger::debug("Upload Texture {} succesfully in slot {}", texture_id, Model::texture_layout_counter);
