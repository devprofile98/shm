...
// macro to replace all '\' in path with '/'
// in mtl file generated by blender for example path to texture
// is constructed with '\' so we have trouble in linux
#define IND_PATH(x)                                                                                                              \
    do {                                                                                                                         \
        std::string::size_type pos = 0;                                                                                          \
        size_t len = strlen(x);                                                                                                  \
        size_t counter = 0;                                                                                                      \
        while (x[counter] != '\0' && counter <= len) {                                                                           \
            if (x[counter] == '\\') {                                                                                            \
                x[counter] = '/';                                                                                                \
            }                                                                                                                    \
            counter += 1;                                                                                                        \
...
struct Color {
    Color(){};
    Color(float red, float green, float blue) : r(red), g(green), b(blue) { normalize(); }
    float r, g, b;
...
const Color ORANGE = {1.0, 1.0, 0.0};
} // namespace rgb
...
class LerpManager {
  public:
    LerpManager() = delete;
    void add(Lerp p) { wait_list.push_back(p); }
